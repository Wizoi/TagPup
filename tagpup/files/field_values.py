...
change", so a cleared field is an explicit '-FIELD=' (tagpup.files.keywords).
"""
...
def write(et, photo_path, values):
    """Write `values` ({field: value}) into a photo, clearing a field whose value is
    empty. The photo's identity goes through tagpup.files.identity, which tells ExifTool
    an objection to the photo's XMP is minor and checks the write cost the keywords
    nothing. Raises when ExifTool fails."""
    params, clear = {}, []
