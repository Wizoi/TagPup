...
not the folders on disk, with a filter box. Each shows every photo under it -- what
removing it takes -- and one gone from disk is marked "not on disk": that is the usual
...
- `/api/folder/indexed`: Returns `{"folders": [{"path", "photos", "on_disk"}]}` — each folder the library holds photos directly in, by path, spelled as the library stores it; `photos` counts every photo under it, which is what removing it takes, and `on_disk` says whether the folder is still there. Remove Folder offers these, not the disk's folders: the usual reason to remove a folder is that it is gone from disk, and a dialog of the disk's folders cannot pick one that is (#47).
