...
- [ ] **The settings dialog** (TagTuner's gear), made from the settings' one declaration -- type, range, whether it is locked, its consequences, its info text -- which the validator reads too (`tagpup.core.validation.SETTINGS` holds the type and range today): each setting with an info button saying what it changes and when. Settings with consequences are locked -- shown, not editable -- and open only through Change..., which lists what changing it does and asks for each to be acknowledged before it is saved; the page reloads after:
