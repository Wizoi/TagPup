...
- [x] An entry point beside web and cli, `python -m tagpup.mcp`, over stdio, in a process of its own; it never talks to a running app's port. There is no default library (#100): every tool names the library it asks about, a name in the home's data folder, and `libraries` lists them. The project's `.mcp.json` starts it for Claude Code sessions in this repository.
...
- [ ] **One validator** *(owner, 2026-09-25)*: `tagpup.core.validation` registers every kind of input -- a tag, a person's name, caption text, a library's name (reserved names included), a rename grouping, a folder, each setting's value (type, range, choices) -- and the four `problem_with_*` checks move into it. Every service checks what it is given before it writes and refuses with the rule's message, so the pages, the CLI and the MCP tools are held to the same rules, and a page that is bypassed still cannot write what the rules refuse. The pages check early from the same rules: the server publishes them as data (`/api/rules`: patterns, forbidden characters, lengths, ranges, choices, messages) and one `web/common/validate.js` applies them, so no page keeps a copy of a rule; a check that is logic, not data, keeps its JavaScript twin, and one file of cases runs through both the Python and the JavaScript tests so the two cannot disagree. Today four checks are written twice, and the library-name rule is a pattern the page repeats without the reserved names. Output is its own half: `web/common/dom.js` builds elements from text, and a guard fails a page that writes a value into `innerHTML` (27 places today, the tag editor's removal check among them).
