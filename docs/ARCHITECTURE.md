...
| 2026-09-25 | Recurring operations (snapshots, sync, pruning the journal, compacting) are registered in `tagpup.jobs.recurring` with their periods and run by one runner inside any TagPup process, or as a background process started at login; their runs are recorded in the library. No Windows Task Scheduler *(owner)*. |
