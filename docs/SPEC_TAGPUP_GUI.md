...
- `/api/folder/rename-photos`: Expects JSON body `{"photo_paths": list, "grouping": string}`. Sequentially renames selected photos based on the custom pattern and grouping template. A grouping holding " - " is refused with `400`: it separates the parts of a photo's name, and editing a caption later splits the name there to find the photo's number. A dash without spaces, as in `2019-06`, is fine.
