...
- **Use Scratch Scripts**: For any non-trivial database query, batch update, or test code, write a temporary Python script to the conversation's scratch directory (`C:\Users\kingersoll\.gemini\antigravity-ide\brain\<conversation-id>\scratch\`) and execute it via `run_command` instead of trying to escape quotes.
...
- **Leaf Name Matching**: The `name` column in `tag_taxonomy` must always store the bare leaf name (e.g., `'Cora Ingersoll'`), not the hierarchical tag path (e.g., `'People/Cora Ingersoll'`).
...
- **ArtifactMetadata Usage**: Only supply `ArtifactMetadata` when editing files inside the active conversation's brain folder (`C:\Users\kingersoll\.gemini\antigravity-ide\brain\<conversation-id>/`). Providing it for repository files causes validation failures.
